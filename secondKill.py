##################################################################################################################
# 抢购脚本自动付款升级本（淘宝为例）（其他请自行修改）                                                                               #                     #
# 使用方法：                                                                                                     #
#     1、先将需要抢购的商品放到购物车中（注意购物车中只能放需要抢购的东西，到时抢购的时候会全部提交）；                         #
#     2、需要用支付宝余额支付
#     3、修改下本脚本中的BUY_TIME值，设定为需要抢购的时间；                                                      #
#     4、执行此脚本，然后等待浏览器打开弹出登陆界面，手机淘宝扫描登陆；                                          #
#     5、脚本会自动进入购物车，全选购物车，自动点击结算，提交订单，输入支付宝密码（你需要自己输入密码），确认付款  #
#     6、网速也是很关键的因素哦！                                                             #
##################################################################################################################
import os
                    ...
                    while True:
                        try:
                            ...
                            if click_submit_times < 50:
                                ...
                                print("已经点击提交订单按钮")
                                print(click_submit_times)
                                submit_succ = True
                                break
                            else:
                                print("提交订单失败...",click_submit_times)
